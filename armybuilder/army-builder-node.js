...
function addUnitToArmy(e, object, pos, merc){
    ...
    if (canThisModelBeAddedToArmy(object) == true) {
        ...
        if (parseInt(object['purchased_low']) < parseInt(object['purchased_high'])) {
            ...
            $.getJSON('/ajax/get-unit-attachments.php?id='+object['id'], function(data) {
                ...
                console.log(merc);
                writeModelAdditionToTempList(object, object['cost'], merc);
...
function writeModelAdditionToTempList(object, cost, count, merc){ // object = unitModel Object // cost of the unit added // count - only on unit min/max choice adds
    cost = parseInt(cost);
    if (merc) {
        var unitCountNum = 1;
        if (object['type'].indexOf('Solo') > -1){
            ...
            tempList['mercArmyModels'].push(soloItem);
        }
        if (object['type'].indexOf('Unit') > -1){
            ...
            tempList['mercArmyModels'].push(unitItem);
        }
    } else {
