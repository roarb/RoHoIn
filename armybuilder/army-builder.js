...
function addToBattleGroup(count, object){ // count = battle group 1-4, object = model object
    object = cleanUnitEntry(object); // currently updating the field_allowance to a numerical value
...
function addLeaderToBattleGroup (count, object){ // count = battlegroup 1-4, object = model object
    ...
    var innerHtml = '<paper-material elevation="1" class="leader" id="model-id-'+object["id"]+'"><span class="unit-name">'+object["name"]+'</span><br /><span class="unit-title">';
    innerHtml += object['title'] + '</span></paper-material>';
...
function addUnitToBattleGroup (count, object){ // count = battlegroup 1-4, object = model object
    ...
    $.getJSON('/ajax/get-unit-attachments.php?id='+object['id'], function(data) {
        ...
        innerHtml += '<paper-material elevation="1" class="child-model model-id-'+object["id"]+'" id="'+modelIdDisplay+'"><span class="unit-name">' + object["name"] + '</span><br /><span class="unit-title">';
        innerHtml += object["title"] + '</span> | <span class="points">' + object["cost"] + '</span> pts';
    ...
    if (canThisModelBeAddedToArmy(object) == true) {
        ...
        if (parseInt(object['purchased_low']) < parseInt(object['purchased_high'])) {
            ...
            $.getJSON('/ajax/get-unit-attachments.php?id='+object['id'], function(data) {
                ...
                var innerHtml = '<span class="wrapper">';
                innerHtml += '<paper-material elevation="1" class="unit-model model-id-'+object["id"]+'" id="'+modelIdDisplay+'"><span class="unit-name">' + object["name"] + '</span><br /><span class="unit-title">';
...
function addUnitAttachmentToArmy(unitId, unitName, unitCost, unitTitle, parentUnit){ // attached unit id, attached unit name, attached unit cost, attached unit title, parent model-id-XX
    ...
    console.log(parentPaperMaterial);
