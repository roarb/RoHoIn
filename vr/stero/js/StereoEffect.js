...
THREE.StereoEffect = function ( renderer ) {
    ...
    this.render = function ( scene, camera ) {
        ...
        var sceneL = renderLeftScene(scene);
        renderer.render( sceneL, _cameraL );

        renderer.setViewport( _width, 0, _width, _height );
        var sceneR = renderRightScene(scene);
        renderer.render( sceneR, _cameraR );

    };

    function renderLeftScene(scene){
        stereoTexture.offset.set(0, 0);
        return scene;
    }

    function renderRightScene(scene){
